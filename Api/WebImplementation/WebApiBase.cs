...
public class WebApiBase
{
    ...
    protected static string Query(ICredential c) => $"?login={c.Login}&password={c.Password}";
    private static string Query(ApiAction action) => $"&action={action}";
    private static string Query(string val) => $"&val={val}";
    protected static string Query(ICredential c, string val) => Query(c) + Query(val);
    protected static string QueryAction(ApiAction action) => $"?action={action}";
    protected static string Query(ICredential c, ApiAction apiAction) => Query(c) + Query(apiAction);
}